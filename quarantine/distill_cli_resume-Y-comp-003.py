...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=21,
        help="The maximum number of files to process in a single batch."
    )
    parser.add_argument(
        "-w", "--max-workers",
        type=int,
...
def create_batches(file_list: List[Path], batch_size: int) -> List[List[Path]]:
    """Splits a list of files into batches with similar total file sizes, respecting max files per batch."""
    if not file_list:
        return []

    # Get file sizes
    file_sizes = [(file, file.stat().st_size) for file in file_list]

    # Sort files by size in descending order
    file_sizes.sort(key=lambda x: x[1], reverse=True)

    # Calculate the minimum number of batches needed
    num_files = len(file_list)
    num_batches = (num_files + batch_size - 1) // batch_size

    # Initialize batches and their current sums
    batches: List[List[Path]] = [[] for _ in range(num_batches)]
    batch_sums = [0] * num_batches

    # Assign each file to the batch with the smallest current sum that has space
    for file, size in file_sizes:
        # Find the batch with the smallest sum that has fewer than batch_size files
        min_sum = float('inf')
        min_idx = -1
        for i in range(num_batches):
            if len(batches[i]) < batch_size and batch_sums[i] < min_sum:
                min_sum = batch_sums[i]
                min_idx = i

        if min_idx == -1:
            raise ValueError("Unable to assign file to a batch. Check batch_size and file counts.")

        batches[min_idx].append(file)
        batch_sums[min_idx] += size

    # Remove any empty batches (though unlikely)
