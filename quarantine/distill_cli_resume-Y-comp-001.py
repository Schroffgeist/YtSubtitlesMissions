...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=21,
        help="The number of files to process in a single batch (used to determine the number of batches)."
    )
    parser.add_argument(
        "-w", "--max-workers",
        type=int,
...
def create_batches(file_list: List[Path], batch_size: int) -> List[List[Path]]:
    """Splits a list of files into batches with roughly equal total file sizes.
    Attempts to limit each batch to at most batch_size files, but may exceed this
    to balance batch sizes.
    """
    if not file_list:
        return []

    # Calculate total number of files and the initial number of batches
    total_files = len(file_list)
    num_batches = max(1, math.ceil(total_files / batch_size))
    ...
    for file_path in file_list:
        try:
            ...
            # If we can't get the size (e.g., file doesn't exist), assume size 0
            file_sizes.append((file_path, 0))

    total_size = sum(size for _, size in file_sizes)
    if total_size == 0:
        # If all files have size 0 (or couldn't be read), fall back to original behavior
        return [file_list[i:i + batch_size] for i in range(0, total_files, batch_size)]
    ...
    batch_counts = [0] * num_batches  # Number of files in each batch

    for file_path, file_size in file_sizes:
        # Find the batch with the smallest current total size that hasn't reached batch_size
        # If all batches have reached batch_size, pick the one with smallest size anyway
        candidate_batches = []
        for batch_idx in range(num_batches):
            candidate_batches.append((batch_sizes[batch_idx], batch_counts[batch_idx], batch_idx))

        # Sort by current size (ascending), then by current count (ascending)
        candidate_batches.sort()

        # Find the first batch that hasn't reached batch_size yet
        target_batch_idx = None
        for size, count, batch_idx in candidate_batches:
            if count < batch_size:
                target_batch_idx = batch_idx
                break

        if target_batch_idx is None:
            # All batches have reached batch_size; pick the one with smallest size
            target_batch_idx = candidate_batches[0][2]

        # Add the file to the target batch
        batches[target_batch_idx].append(file_path)
        batch_sizes[target_batch_idx] += file_size
        batch_counts[target_batch_idx] += 1
...
def main():
    ...
    # Resolve paths at the beginning to ensure absolute paths are used everywhere.
