...
def get_args():
    ...
    parser.add_argument(
        "--max-batch-size",
        type=int,
        default=10000000,  # 10MB default
        help="Maximum total size (in bytes) for a single batch."
    )
    parser.add_argument(
        "-w", "--max-workers",
        type=int,
...
def get_file_sizes(file_list: List[Path]) -> List[Tuple[Path, int]]:
    """Returns a list of (file_path, file_size) tuples."""
    return [(f, f.stat().st_size) for f in file_list]

def create_size_batches(files_with_sizes: List[Tuple[Path, int]], max_batch_size: int, target_batch_count: int) -> List[List[Path]]:
    """Creates batches based on file sizes to evenly distribute total size."""
    ...
    # Calculate target batch size based on total size and desired batch count
    total_size = sum(size for _, size in files_with_sizes)
    target_batch_size = max(total_size // target_batch_count, max_batch_size // 10)
    ...
    for file_path, file_size in files_with_sizes:
        # If file is larger than max_batch_size, put it in its own batch
        ...
        # If adding this file would exceed max batch size, start a new batch
        ...
        # If current batch is close to target size, start a new batch
        if current_batch_size >= target_batch_size and len(batches) < target_batch_count - 1:
            batches.append(current_batch)
            current_batch = []
            current_batch_size = 0
...
def create_batches(file_list: List[Path], batch_size: int, max_batch_size: int) -> List[List[Path]]:
    """Splits files into batches considering both count and size limits."""
    if not file_list:
        return []
    
    files_with_sizes = get_file_sizes(file_list)
    return create_size_batches(files_with_sizes, max_batch_size, batch_size)
...
def run_distillation_round(input_files: List[Path], round_number: int, output_path: Path, batch_size: int, max_batch_size: int, max_workers: int, model: str, prompt: str, timeout: int, resume: bool) -> List[Path] or None:
    """
    ...
    batches = create_batches(input_files, batch_size, max_batch_size)
    print(f"Divided into {len(batches)} batches based on size distribution.")
    
    # Calculate and display batch size statistics
    files_with_sizes = get_file_sizes(input_files)
    total_size = sum(size for _, size in files_with_sizes)
    avg_batch_size = total_size / len(batches) if batches else 0
    print(f"Total size: {total_size} bytes, Average batch size: {avg_batch_size:.2f} bytes")
...
def main():
    ...
    # Calculate and display initial file size statistics
    files_with_sizes = get_file_sizes(initial_files)
    total_size = sum(size for _, size in files_with_sizes)
    avg_size = total_size / len(initial_files) if initial_files else 0
    max_size = max(size for _, size in files_with_sizes) if initial_files else 0
    print(f"Total size: {total_size} bytes, Average file size: {avg_size:.2f} bytes, Largest file: {max_size} bytes")
    ...
    while len(current_files) > 1:
        distilled_files = run_distillation_round(
            ...
            args.batch_size, args.max_batch_size, args.max_workers, args.model, prompt, args.timeout, args.resume
        )
